...
import { Building2, HelpCircle, Save, Send, Settings, Upload } from "lucide-react";
    ...
    try {
      const response = await fetch("/api/obras", {
        ...
        body: JSON.stringify({
          ...
          eixoId: secretaria.eixo?.id ?? null, engenheiroId: form.engenheiroId || null, status: "PLANEJADA",
        }),
      });
    ...
    <div className={styles.shell}>
      ...
      <div className={styles.body}>
        ...
        <main className={styles.main}>
          <div className={styles.pageHeading}><div><div className={styles.breadcrumb}>Obras e Projetos <span>/</span> Novo cadastro</div><h1 className={styles.pageTitle}>Cadastrar Nova Obra Pública <em>NOVO REGISTRO</em></h1><p>Preencha os dados técnicos, orçamentários e contratuais para registrar o projeto na base municipal de infraestrutura.</p></div><div className={styles.headingActions}><button className={styles.secondaryButton} type="button" onClick={() => { setStatus("saved"); setMessage("Rascunho salvo localmente."); }}><Save size={16} /> Salvar como Rascunho</button><button className={styles.primaryButton} type="submit" form="obra-form"><Send size={16} /> Publicar</button></div></div>
          <form id="obra-form" className={styles.form} onSubmit={handleSubmit}>
            <section className={styles.card}><div className={styles.cardHeading}><div><span className={styles.step}>01</span><div><h2>Identificação e escopo da obra</h2><p>Campos com (*) são obrigatórios</p></div></div></div><div className={styles.fieldsGrid}>
              ...
              <Field label="Secretaria responsável"><input value={`${secretaria.sigla} - ${secretaria.nome}`} readOnly /></Field>
              <Field label="Eixo estratégico"><input value={secretaria.eixo?.nome ?? "Sem eixo vinculado"} readOnly /></Field>
            ...
            <section className={styles.card}><div className={styles.cardHeading}><div><span className={styles.step}>03</span><div><h2>Responsabilidade técnica</h2><p>Defina os responsáveis pelo acompanhamento da execução.</p></div></div></div><div className={styles.fieldsGrid}><Field label="Engenheiro fiscal titular"><select value={form.engenheiroId} onChange={(event) => update("engenheiroId", event.target.value)}><option value="">Selecione o responsável técnico</option>{engenheiros.map((engenheiro) => <option key={engenheiro.id} value={engenheiro.id}>{engenheiro.nome}</option>)}</select></Field></div></section>
