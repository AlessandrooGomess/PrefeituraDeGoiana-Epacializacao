...
export function IntercorrenciasCard({
  ...
  const isPertoDoLimite = caracteresRestantes <= 100;
  const isLimiteExcedido = caracteresRestantes < 0;

  return (
    <div
      className={`rounded-xl border bg-white p-4 shadow-xs transition-colors ${
        erro || isLimiteExcedido ? "border-rose-400 bg-rose-50/20" : "border-slate-200"
      }`}
    >
      ...
      <div className="mt-4 space-y-1.5">
        ...
        {erro && <p className="text-[11px] font-medium text-rose-600">{erro}</p>}
      </div>
    </div>
  );
}